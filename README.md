...
- **本日議題**：獨立卡片（位於開場亮點之前、**簡報第 1 頁**），可放自訂大綱／議題；一般檢視點「✎ 編輯議題」輸入（一行一項）後按「儲存」定案，內容為空時顯示「＋ 新增本日議題」入口；議題**隨「存檔（含資料）」副本與自動記憶一起保存**，載入新檔案不被覆蓋，「清除記憶」或「清除議題」才會移除。卡片為兩欄：**右欄「開會資訊」**即時顯示本份數據的分析期間與案件件數，並附**本月每日新增案件迷你折線圖（sparkline）**（月份錨點＝單月期間取該月、其餘回退目前月；折線下方標示日期數字），隨期間同步更新，簡報第 1 頁也一併呈現。
