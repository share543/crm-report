...
- **簡報模式**：開會投影用，工具列「開始簡報」進入**全螢幕投影片**，一次一個區段（**本日議題**→開場亮點→統計→各課簡表→進度→類型→漏斗→排行→開發時間→轉換流失→待留意→案件清單），`←/→`／空白／`PgUp/PgDn` 換頁、`Home/End` 跳頭尾、`Esc` 結束；底部工具列可**即時切換分析期間**（整份報表同步縮放），`D` 鍵可直接操作期間下拉。
- **本日議題**：獨立卡片（位於開場亮點之前、**簡報第 1 頁**），可放自訂大綱／議題；一般檢視點「✎ 編輯議題」輸入（一行一項）後按「儲存」定案，內容為空時顯示「＋ 新增本日議題」入口；議題**隨「存檔（含資料）」副本與自動記憶一起保存**，載入新檔案不被覆蓋，「清除記憶」或「清除議題」才會移除。
